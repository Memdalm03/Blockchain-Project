...
# Builds the list of Node objects (honest + byzantine) for a simulation run.
def create_network(total_nodes, byzantine_count, honest_values=None):
    ...
    if honest_values is None:
        honest_values = ["Bob transferred $500 to Alice"]
...
def print_summary(results):
    ...
    for r in results:
        ...
        print(f"{r['scenario']:<18}{r['total_nodes']:<5}{r['t']:<5}{r['byzantine_count']:<5}"
              f"{'YES' if r['formula_valid'] else 'NO':<9}"
    ...
    print("\nNotes:"
          ...
          "\n  Byzantine nodes always send corrupted (equivocating) symbol pairs")


SCENARIOS = [
    {"scenario_name": "Valid n=7 t=2", "total_nodes": 7, "t": 2, "byzantine_count": 2},
    {"scenario_name": "Valid n=10 t=3", "total_nodes": 10, "t": 3, "byzantine_count": 3},
    {"scenario_name": "Valid n=13 t=4", "total_nodes": 13, "t": 4, "byzantine_count": 4},
